...
process_gee_data <- function(folder_path, pop_source) {
  list.files(folder_path, pattern = "\\.csv$", full.names = TRUE) |>
    map_dfr(
      read_csv,
      col_types = cols(shapeGroup = col_character()),
    ...
    mutate(
      ...
      across(
        starts_with("pop2000"),
        ~ .x / pop2000_total * 100,
        .names = paste0("{.col}_", pop_source, "_pct")
      ),
      across(
        starts_with("pop2020"),
        ~ .x / pop2020_total * 100,
...
pa_pop_combined <- full_join(
  ...
  rename(
    country_area_km2 = country_area_km2_worldpop,
  ...
  # and remove the ghsl
  select(
    -country_area_km2_ghsl,
    -pa_area_2000_km2_ghsl,
    -pa_area_2020_km2_ghsl,
    -pa_area10km_2000_km2_ghsl,
    pa_area10km_2020_km2_ghsl
  )
...
pa_pop_summary_with_total <- bind_rows(
  pa_pop_total,
  ...
  select(
    ...
    pa_area10km_2000_km2_pct,
    pa_area10km_2020_km2_pct,
...
pa_pop_summary_with_total %>%
  ...
  cols_label(
    ...
    pop2000_in_pa_worldpop_pct = "2000",
    pop2020_in_pa_worldpop_pct = "2020",
    pop2000_in_pa_ghsl_pct = "2000",
    pop2020_in_pa_ghsl_pct = "2020",
    pa_area10km_2000_km2_pct = "2000",
    pa_area10km_2020_km2_pct = "2020",
    pop2000_in_pa10_worldpop_pct = "2000",
    pop2020_in_pa10_worldpop_pct = "2020",
  ...
  tab_source_note(
    source_note = "Source: Analysis based on WDPA, WorldPop & GHSL data (2000-2020)"
  ) %>%
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = c(country, pa_area10km_2020_km2_pct, pop2020_in_pa10_ghsl_pct)
    )
  ) %>%
...
pa_pop_summary2 <- pa_pop_worldpop |>
  mutate(
    var_land = (pa_area_2020_km2_pct - pa_area_2000_km2_pct) /
      pa_area_2000_km2_pct,

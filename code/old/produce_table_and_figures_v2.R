...
pa_pop <- new_joint_all |>
  filter(ISO3 %in% llm_2020$ISO3) |>
  arrange(country)

# Sanity checks on expected columns (explicit, readable) -------------------
...
required_cols <- c(
  ...
  "pa_area_strict_2000_km2",
  "pa_area_strict_2020_km2",
  "pa_area_non_2000_km2",
  "pa_area_non_2020_km2",
  "pa_area_all_2000_km2",
  "pa_area_all_2020_km2",

  "pa_area10km_strict_2000_km2",
  "pa_area10km_strict_2020_km2",
  "pa_area10km_non_2000_km2",
  "pa_area10km_non_2020_km2",
  "pa_area10km_all_2000_km2",
  "pa_area10km_all_2020_km2",

  "pa_area_strict_2000_km2_pct",
  "pa_area_strict_2020_km2_pct",
  "pa_area_non_2000_km2_pct",
  "pa_area_non_2020_km2_pct",
  "pa_area_all_2000_km2_pct",
  "pa_area_all_2020_km2_pct",

  "pa_area10km_strict_2000_km2_pct",
  "pa_area10km_strict_2020_km2_pct",
  "pa_area10km_non_2000_km2_pct",
  "pa_area10km_non_2020_km2_pct",
  "pa_area10km_all_2000_km2_pct",
  "pa_area10km_all_2020_km2_pct",

  "pop2000_in_pa_strict_WP",
  "pop2020_in_pa_strict_WP",
  "pop2000_in_pa_non_WP",
  "pop2020_in_pa_non_WP",
  "pop2000_in_pa_all_WP",
  "pop2020_in_pa_all_WP",

  "pop2000_in_pa_strict_GHSL",
  "pop2020_in_pa_strict_GHSL",
  "pop2000_in_pa_non_GHSL",
  "pop2020_in_pa_non_GHSL",
  "pop2000_in_pa_all_GHSL",
  "pop2020_in_pa_all_GHSL",

  "pop2000_in_pa10_strict_WP",
  "pop2020_in_pa10_strict_WP",
  "pop2000_in_pa10_non_WP",
  "pop2020_in_pa10_non_WP",
  "pop2000_in_pa10_all_WP",
  "pop2020_in_pa10_all_WP",

  "pop2000_in_pa10_strict_GHSL",
  "pop2020_in_pa10_strict_GHSL",
  "pop2000_in_pa10_non_GHSL",
  "pop2020_in_pa10_non_GHSL",
  "pop2000_in_pa10_all_GHSL",
  "pop2020_in_pa10_all_GHSL",

  "pop2000_in_pa_strict_WP_pct",
  "pop2020_in_pa_strict_WP_pct",
  "pop2000_in_pa_non_WP_pct",
  "pop2020_in_pa_non_WP_pct",
  "pop2000_in_pa_all_WP_pct",
  "pop2020_in_pa_all_WP_pct",

  "pop2000_in_pa_strict_GHSL_pct",
  "pop2020_in_pa_strict_GHSL_pct",
  "pop2000_in_pa_non_GHSL_pct",
  "pop2020_in_pa_non_GHSL_pct",
  "pop2000_in_pa_all_GHSL_pct",
  "pop2020_in_pa_all_GHSL_pct",

  "pop2000_in_pa10_strict_WP_pct",
  "pop2020_in_pa10_strict_WP_pct",
  "pop2000_in_pa10_non_WP_pct",
  "pop2020_in_pa10_non_WP_pct",
  "pop2000_in_pa10_all_WP_pct",
  "pop2020_in_pa10_all_WP_pct",

  "pop2000_in_pa10_strict_GHSL_pct",
  "pop2020_in_pa10_strict_GHSL_pct",
  "pop2000_in_pa10_non_GHSL_pct",
  "pop2020_in_pa10_non_GHSL_pct",
  "pop2000_in_pa10_all_GHSL_pct",
  "pop2020_in_pa10_all_GHSL_pct"
)

missing_cols <- setdiff(required_cols, names(pa_pop))
if (length(missing_cols) > 0) {
  stop(paste0(
    "Missing required columns in consolidated dataset:\n",
    paste(missing_cols, collapse = "\n")
  ))
}

# Legacy objects used by unchanged outputs (all PAs only) ------------------
...
# Totals (legacy all PAs, unchanged downstream dependencies) ---------------
...
compute_totals <- function(data, label) {
  data |>
    ...
    mutate(
      ...
      pop2000_in_pa_worldpop_pct = pop2000_in_pa_worldpop /
        pop2000_total_worldpop *
        100,
      pop2020_in_pa_worldpop_pct = pop2020_in_pa_worldpop /
        pop2020_total_worldpop *
        100,
      ...
      pop2000_in_pa10_worldpop_pct = pop2000_in_pa10_worldpop /
        pop2000_total_worldpop *
        100,
      pop2020_in_pa10_worldpop_pct = pop2020_in_pa10_worldpop /
        pop2020_total_worldpop *
        100,
      pop2000_in_pa10_ghsl_pct = pop2000_in_pa10_ghsl /
        pop2000_total_ghsl *
        100,
...
# Totals by PA type (strict, non, all) computed from counts ----------------

compute_totals_type <- function(
  data,
  label,
  pa_type = c("strict", "non", "all")
) {
  ...
  data |>
    summarise(
      ...
      pa_type = pa_type,
      country_area_km2 = sum(country_area_km2, na.rm = TRUE),
      pop2000_total_WP = sum(pop2000_total_WP, na.rm = TRUE),
      pop2020_total_WP = sum(pop2020_total_WP, na.rm = TRUE),
      pop2000_total_GHSL = sum(pop2000_total_GHSL, na.rm = TRUE),
      pop2020_total_GHSL = sum(pop2020_total_GHSL, na.rm = TRUE),

      pa_area_2000_km2 = sum(
        .data[[paste0("pa_area_", pa_type, "_2000_km2")]],
        na.rm = TRUE
      ),
      pa_area_2020_km2 = sum(
        .data[[paste0("pa_area_", pa_type, "_2020_km2")]],
        na.rm = TRUE
      ),

      pop2000_in_pa_GHSL = sum(
        .data[[paste0("pop2000_in_pa_", pa_type, "_GHSL")]],
        na.rm = TRUE
      ),
      pop2020_in_pa_GHSL = sum(
        .data[[paste0("pop2020_in_pa_", pa_type, "_GHSL")]],
        na.rm = TRUE
      ),

      pop2000_in_pa10_GHSL = sum(
        .data[[paste0("pop2000_in_pa10_", pa_type, "_GHSL")]],
        na.rm = TRUE
      ),
      pop2020_in_pa10_GHSL = sum(
        .data[[paste0("pop2020_in_pa10_", pa_type, "_GHSL")]],
        na.rm = TRUE
      )
    ) |>
    mutate(
...
# Produce Table S1 (updated, strict vs non vs all, readable width) ---------
...
table_s1_df <- bind_rows(
  ...
  pa_pop |>
    transmute(
      ...
      pop2000_in_pa_GHSL_pct = pop2000_in_pa_strict_GHSL_pct,
      pop2020_in_pa_GHSL_pct = pop2020_in_pa_strict_GHSL_pct
    ),
  pa_pop |>
    transmute(
      country,
      ...
      pop2000_in_pa_GHSL_pct = pop2000_in_pa_non_GHSL_pct,
      pop2020_in_pa_GHSL_pct = pop2020_in_pa_non_GHSL_pct
    ),
  pa_pop |>
    transmute(
      country,
...
table_s1 <- table_s1_df |>
  select(
    ...
    pa_area_2020_km2_pct,
    pop2000_in_pa10_GHSL_pct,
    pop2020_in_pa10_GHSL_pct,
    pop2000_in_pa_GHSL_pct,
    pop2020_in_pa_GHSL_pct
  ) |>
  gt(groupname_col = "pa_type") |>
  tab_header(
    title = "Table S1: PA Coverage and Population Proximity by PA Type (2000 and 2020)",
    subtitle = "Percentages by type. Strict and non strict 10km buffer populations may overlap and are not additive."
  ) |>
  ...
  cols_label(
    ...
    pop2000_in_pa_GHSL_pct = "2000",
    pop2020_in_pa_GHSL_pct = "2020"
  ) |>
  tab_spanner(
  ...
  tab_spanner(
    label = "% population within PAs (GHSL)",
  ...
  tab_source_note(
    source_note = "Source: Consolidated country level dataset from WDPA, WorldPop and GHSL (2000 and 2020)."
  ) |>
  tab_style(
  ...
  tab_style(
    ...
    locations = cells_body(
      rows = country %in% c("Total", "Total without India")
    )
  )
...
smd_results <- tibble(
  ...
  `2000` = c(
    compute_smd(
      pa_pop_combined$pop2000_in_pa_worldpop,
      pa_pop_combined$pop2000_in_pa_ghsl
    ),
    compute_smd(
      pa_pop_combined$pop2000_in_pa10_worldpop,
      pa_pop_combined$pop2000_in_pa10_ghsl
    )
  ),
  `2020` = c(
    compute_smd(
      pa_pop_combined$pop2020_in_pa_worldpop,
      pa_pop_combined$pop2020_in_pa_ghsl
    ),
    compute_smd(
      pa_pop_combined$pop2020_in_pa10_worldpop,
      pa_pop_combined$pop2020_in_pa10_ghsl
    )
  )
)
...
# Produce Table S2 (unchanged logic) -------------------------------------

pa_pop_diff <- pa_pop_combined |>
  select(
    ISO3,
    country,
    pop2000_in_pa_worldpop_pct,
    pop2020_in_pa_worldpop_pct,
  ...
  mutate(abs_diff = abs(worldpop - ghsl), diff_perc = (abs_diff / ghsl))
...
table_s2 <- major_discrepancies |>
  ...
  tab_header(
    ...
    subtitle = md(
      "*Absolute difference of more then 5 percentage points (pp) and relative difference of more than 10%*"
    )
  ) |>
  cols_label(
    country = "Country",
...
fig2_df <- bind_rows(
  pa_pop |>
    transmute(
      ...
      pa_type = "Strict",
      pop2020_total = pop2020_total_WP,
      var_land = (pa_area_strict_2020_km2_pct - pa_area_strict_2000_km2_pct) /
        pa_area_strict_2000_km2_pct,
      var_pop = (pop2020_in_pa10_strict_WP_pct -
        pop2000_in_pa10_strict_WP_pct) /
        pop2000_in_pa10_strict_WP_pct,
  ...
  pa_pop |>
    transmute(
      ...
      pa_type = "Non strict",
      pop2020_total = pop2020_total_WP,
      var_land = (pa_area_non_2020_km2_pct - pa_area_non_2000_km2_pct) /
        pa_area_non_2000_km2_pct,
      var_pop = (pop2020_in_pa10_non_WP_pct - pop2000_in_pa10_non_WP_pct) /
        pop2000_in_pa10_non_WP_pct,
...
x_lim <- range(fig2_df$var_land, finite = TRUE)
y_lim <- range(fig2_df$var_pop, finite = TRUE)
lim_all <- range(c(x_lim, y_lim), finite = TRUE)

legend_line <- data.frame(
  x = c(lim_all[1], lim_all[2]),
  y = c(lim_all[1], lim_all[2]),
  type = "Equal PA & Population Growth"
)
...
figure_2 <- ggplot(fig2_df, aes(x = var_land, y = var_pop, label = ISO3)) +
  ...
  scale_linetype_manual(
    name = NULL,
    values = c("Equal PA & Population Growth" = "solid")
  ) +
  scale_size_continuous(
    breaks = c(10, 100, 200),
    labels = c("10", "100", "200"),
    guide = guide_legend(
      title = "Total Population in 2020 (millions)",
      title.position = "top"
    )
  ) +
  ...
  labs(
    title = "Figure 2: Growth in PA land share vs growth in population within 10km (WorldPop 2000-2020)",
    x = "Growth in Land Share Covered by PAs",
    y = "Growth in Population Share Within 10km of PAs",
    subtitle = paste0(
      "Baseline threshold: PA land share for the relevant type above ",
      baseline_threshold_pct,
      "% in 2000"
    ),
    caption = str_wrap(paste("Country codes:", country_note), width = 140)
  ) +
  theme_minimal() +
  theme(
...
figure_3 <- ggplot(
  fig3_df,
  aes(x = x, y = y, size = pop2020_total / 1e6, label = ISO3)
) +
  ...
  labs(
    title = "Figure 3: PA Coverage vs Population Near PAs (2020, WorldPop)",
...
figure_4 <- pa_pop_diff %>%
  ...
  geom_segment(
    aes(x = ghsl, xend = worldpop, yend = pa_area_pct),
    color = "blue",
    linewidth = 1
  ) +
  ...
  facet_grid(
    ...
    cols = vars(year),
    labeller = labeller(
      metric = c("pa" = "Within PAs", "pa10" = "Within 10km of PAs")
    )
  ) +
  scale_color_manual(
...
table_s3 <- table_s3_df |>
  ...
  cols_label(
    ...
    land_2000 = "2000",
    land_2020 = "2020",
    land_change_pp = "Change (pp)",
    pop10_2000 = "2000",
    pop10_2020 = "2020",
    pop10_change_pp = "Change (pp)"
  ) |>
  tab_spanner(
    label = "Land protected (%)",
    columns = c(land_2000, land_2020, land_change_pp)
  ) |>
  tab_spanner(
    label = "Population within 10km (%)",
    columns = c(pop10_2000, pop10_2020, pop10_change_pp)
  ) |>
  cols_align(align = "center", columns = everything()) |>
  tab_options(
    table.font.size = px(11),

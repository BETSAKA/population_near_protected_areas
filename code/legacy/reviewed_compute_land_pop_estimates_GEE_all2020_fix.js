 ...
 *
 * Minimal fix relative to reviewed_compute_land_pop_estimates_GEE.js:
 * 1. Add an explicit All_2020 scenario.
 * 2. Filter WDPA polygons to the focal ISO3 before buffering/rasterization.
 * 3. Export to a separate folder so existing reviewed outputs are untouched.
 *
 * Use the All_2020 rows from this script directly.
 * Do not rebuild All_2020 by adding Confirmed_2020 + Unknown_Year in R.
 */
...
var MAX_PIXELS = 1e13;
var TILE_SCALE = 16;
var EXPORT_FOLDER = 'PA_Pop_Final_Absolute_All2020_Fix';
...
function processRegion(feature, admLevel, iso, source) {
  ...
  var scenarios = [
    ...
    {id: 'All_2020',       popYear: 2020, filter: ee.Filter.or(
...
SOURCES.forEach(function(source) {
  ISO3_LIST.forEach(function(iso) {
    ...
    Export.table.toDrive({
      collection: results, description: 'PA_Pop_' + iso + '_' + source + '_All2020Fix',

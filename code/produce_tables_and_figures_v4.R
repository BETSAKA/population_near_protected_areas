...
llm_2020 <- wb_country_list |>
  ...
  filter(!ISO3 %in% c("TLS", "SSD"))

# Load new GEE outputs ----------------------------------------------------

new_joint_files_all <- list.files(
  "data/GEE_PA_Population_Stats",
  pattern = "\\.csv$",
  full.names = TRUE
)
...
required_cols <- c(
  "iso3", "adm_level", "country_name", "country_area_km2", "adm_area_km2",
  "pop2000_total_WP", "pop2020_total_WP", "pop2000_total_GHSL", "pop2020_total_GHSL",
  "pa_area_strict_2000_km2", "pa_area_strict_2020_km2",
  "pa_area10km_strict_2000_km2", "pa_area10km_strict_2020_km2",
  "pa_area_all_2000_km2", "pa_area_all_2020_km2",
  "pa_area10km_all_2000_km2", "pa_area10km_all_2020_km2",
...
if (length(missing_required) > 0) {
  stop(paste0(
    "Missing required columns in GEE outputs:\n",
...
# Rule per ISO3:
# Either ADM0 mode: adm_level all 0 and exactly one row
# Or ADM1 mode: adm_level all 1 and no duplicate adm1_name, and one row per adm1_name
# Mixed adm_level within ISO3 is an error

check_iso3_structure <- function(df) {
  problems <- df |>
    group_by(iso3) |>
    summarise(
      n_rows = n(),
      n_levels = n_distinct(adm_level),
      levels = paste(sort(unique(adm_level)), collapse = ","),
      n_adm1_name = sum(!is.na(adm1_name) & adm1_name != ""),
      n_distinct_adm1_name = n_distinct(adm1_name),
      any_dup_adm1_name = any(duplicated(adm1_name[!is.na(adm1_name) & adm1_name != ""])),
      .groups = "drop"
    ) |>
    mutate(
      is_adm0 = (n_levels == 1 & levels == "0"),
      is_adm1 = (n_levels == 1 & levels == "1"),
      ok_adm0 = is_adm0 & n_rows == 1,
      ok_adm1 = is_adm1 & !any_dup_adm1_name,
      ok = ok_adm0 | ok_adm1
    ) |>
    filter(!ok)
  ...
  if (nrow(problems) > 0) {
    msg <- problems |>
      transmute(
        ...
        levels,
        any_dup_adm1_name,
        n_distinct_adm1_name
      ) |>
      mutate(line = paste0(
        iso3, ": n_rows=", n_rows,
        ", adm_level=", levels,
        ", dup_adm1_name=", any_dup_adm1_name,
        ", n_distinct_adm1_name=", n_distinct_adm1_name
      )) |>
      pull(line)
    
    stop(paste0(
      "Inconsistent or duplicated admin rows detected.\n",
      paste(msg, collapse = "\n")
    ))
  }
...
check_iso3_structure(raw)

# Aggregate to country level ----------------------------------------------
# Sum all numeric columns except admin descriptors and country area
# Keep country_area_km2 as first (should be constant within ISO3)
...
# Sanity check: base columns for percent computations ---------------------
...
if (length(missing_base) > 0) {
  stop(paste0(
    "Missing required base columns after aggregation:\n",
...
# Legacy objects for unchanged outputs (All only) -------------------------
...
# Totals rows (All only, unchanged dependencies) --------------------------
...
# Totals by type (Strict and All only) ------------------------------------

compute_totals_type <- function(data, label, pa_type = c("strict", "all")) {
...
save(
  pa_pop_worldpop, pa_pop_ghsl, pa_pop_total, pa_pop_total_no_india,
  file = "results/pa_pop_total.rds"
)

# Produce Table S1 (Strict and All) ---------------------------------------
...
table_s1_df <- bind_rows(
  ...
  mutate(
    pa_type = factor(pa_type, levels = c("strict", "all"),
                     labels = c("Strict", "All"))
  ) |>
  arrange(
...
table_s1 <- table_s1_df |>
  ...
  tab_header(
    ...
    subtitle = "Strict and All 10km buffer populations may overlap and are not additive."
  ) |>
...
# Produce Table S2 (unchanged logic, All only) ----------------------------
...
# Produce Figure 1 (unchanged logic, All only) ----------------------------
...
figure_2 <- ggplot(fig2_df, aes(x = var_land, y = var_pop, label = ISO3)) +
  ...
  labs(
    ...
    subtitle = paste0("Baseline filter: strict land share above ", baseline_threshold_pct, "% in 2000")
  ) +
  theme_minimal() +
  theme(
...
save_plot("results/figure_2.png", plot = figure_2, base_width = 8, base_height = 6.5, unit = "in", dpi = 300)

# Produce Figure 3 (Strict only) ------------------------------------------
...
figure_3 <- ggplot(fig3_df, aes(x = x, y = y, size = pop2020_total / 1e6, label = ISO3)) +
  ...
  labs(
    title = "Figure 3: Strict PA land share vs population within 10km in 2020 (WorldPop)",
    x = "% of country covered by strict PAs",
    y = "% of population within 10km of strict PAs"
  ) +
  theme_minimal() +
  theme(
...
save_plot("results/figure_3.png", plot = figure_3, base_width = 8, base_height = 6.5, unit = "in", dpi = 300)

# Produce Figure 4 (unchanged logic, All only) ----------------------------
...
# Produce Figure 5 (Strict vs All, non additive) --------------------------
...
figure_5 <- ggplot(fig5_long, aes(x = year, y = pct, fill = pa_type)) +
  ...
  labs(
    title = "Figure 5: Population within 10km of PAs by type (GHSL, 2000 and 2020)",
    x = NULL,
    y = "% of population within 10km",
    fill = "PA type",
    caption = "Strict and All 10km buffers may overlap. Bars are not additive."
  ) +
  theme_minimal() +
  theme(
    legend.position = "bottom",
...
# Produce Table S3 (totals by type, Strict and All) ------------------------
